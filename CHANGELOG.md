...
### Optimized
- Reduced the Core Erlang and BEAM footprint of N-gram window generation.
- Reduced allocations and reductions in `FastNgram.word_ngrams/2` by sliding over the existing word list and rejecting oversized windows before joining words.
- Preserved efficient oversized-window rejection for non-ASCII `FastNgram.letter_ngrams/2` inputs.

### Added
- Added regression coverage for oversized Unicode letter N-grams.
